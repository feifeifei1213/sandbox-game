...
| I4-01 | 明确线上订单与市场竞标正式需求口径 | P0 | 已完成 | 用户 2026-05-18/2026-05-19 确认、`道具-订单推算（服务企业）.xlsx` | `docs/requirements_spec.md`、`docs/requirements_consensus_checklist.md`、`docs/minimal_state_machine.md`、`docs/calculation_rule_spec.md`、`docs/excel_field_mapping.md`、`docs/api_design.md`、`docs/database_design.md` | 已明确 `0年` 不需要订单；`1年` 起按 `市场 + 订单类型` 形成最多 `16` 个独立标段；管理员开标前配置全年标段释放顺序且开标中不可调整；每标段一轮选单、每组最多一单；`1年` 按市场投入排序，`2年` 起市场龙头优先，市场龙头即使本年对应市场 `0` 投入也可优先；非龙头同投入按上一年度该市场订单总额再随机；玩家可放弃，管理员可代跳过但不能代选；选单状态 `3` 秒自动轮询；交付绑定季度且销售收入需等于交付订单金额合计；未交付只记录状态，处罚暂不做 | 已完成文档沉淀，尚未进入代码实现 |
| I4-02 | 实现管理员订单管理：Excel 上传、数量配置、标段释放顺序、订单池生成 | P0 | 已完成 | `I4-01`、`docs/api_design.md`、`docs/database_design.md` | `migrations/mysql/0005_order_admin.sql`、`internal/enum/order.go`、`internal/model/entity/order.go`、`internal/repository/order_repository.go`、`internal/service/order_excel_parser.go`、`internal/service/admin_order_service.go`、`internal/http/dto/admin_order_dto.go`、`internal/http/handler/admin_order_handler.go`、`internal/app/router.go`、`frontend/src/api/sandbox-game/admin-order.ts`、`frontend/src/stores/admin-order.ts`、`frontend/src/views/sandbox-game/admin/order/AdminOrderPage.vue`、`frontend/src/router/index.ts`、`frontend/src/components/sandbox-game/admin/AdminNav.vue`、`frontend/src/types/sandbox-game-admin.ts` | 管理员可上传订单推算 Excel，按 `年份 + 市场 + 订单类型` 配置订单数量和释放顺序，生成固定订单池；订单池生成后锁定配置，未开标且未选单时可通过覆盖重新生成；配置更新会清理未开始的旧订单池与旧标段状态；系统展示订单数量风险提示但不强制保底；首版不包含玩家抢单页、标段释放控制与选单顺序生成 | 已通过 `go test ./...` 与 `frontend/npm.cmd run build` 验证；下一步进入 `I4-03` 玩家年度订单页与标段释放/选单流程 |
| I4-03 | 实现玩家年度订单页：市场投入、标段释放、选单顺序、选择/放弃订单 | P0 | 未开始 | `I4-02`、`docs/minimal_state_machine.md` | 后端 `player-order` 接口、玩家年度订单页、市场投入与标段选单状态展示、自动轮询 | 玩家可提交各市场投入，按管理员释放的当前标段和完整排序选择或放弃订单；普通小组未投入市场不参与，市场龙头 `0` 投入仍可参与；已选订单灰色锁定；每标段每组最多选一单；选择后不可撤销 | 待开发 |
| I4-04 | 打通订单模块与经营页规则联动 | P0 | 未开始 | `I4-03`、`docs/calculation_rule_spec.md` | 经营页查询/提交规则改造、订单总额与市场投入汇总、订单交付状态回写 | 正式年份未完成订单前置时拦截 `Q1` 提交；已选订单金额汇总进入订单总额；市场投入汇总进入市场投入；直接成本仍由玩家填写；账期只展示；交付时校验季度销售收入等于交付订单金额合计，年末未交付标记 `UNFINISHED` 且不自动处罚 | 待开发 |
| I4-05 | 补订单模块测试、联调脚本与原型/页面验收 | P1 | 未开始 | `I4-02`、`I4-03`、`I4-04` | 集成测试、`.http` 脚本、管理员/玩家页面验收清单 | 覆盖 `0年` 无订单、无投入且无龙头市场跳过、无投入但有龙头市场仅龙头参与、投入并列按上年市场订单额再随机、市场龙头优先、标段释放顺序锁定、管理员不能跳序、每标段一轮选单、玩家放弃、管理员代跳过、Q1 前置拦截、订单交付校验、未交付状态保留等场景 | 待开发 |
