...
| I4-07 | 增加订单专项测试场景造数能力 | P0 | 未开始 | `I4-06`、用户 2026-05-21 确认 | `cmd/dbtool` 测试场景命令、测试库造数说明、订单专项人工验收步骤 | 新增仅用于开发 / 测试 / 验收的场景初始化能力：不关闭经营页与财报页校验，不改变正式比赛规则，不写入正式比赛主链；脚本在独立测试库初始化 3 个小组，写入合法但非真实的 `0年 / 1年` 经营与平衡财报结果，造出 `1年` 各市场订单选择结果以形成 `2年` 市场龙头，并将流程停在 `2年订单竞标前`；用于人工验证市场开启、未开启市场投入 `0` 拦截、订单生成、标段释放、市场龙头优先、订单锁定和 `Q1` 前置联动 | 下一步先设计造数口径、清理 / 重置方式和防误用限制，再实现 `dbtool` 命令；必须明确只能用于测试库，禁止在正式库执行 |
