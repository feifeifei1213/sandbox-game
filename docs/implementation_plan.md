...
| I7-03 | 实现自动快照生成服务 | P0 | 未开始 | `I7-02` | 快照构建服务、自动触发点接入、单元测试 | 经营阶段提交、财报提交、退回前、订单池确认、标段完成、开放下一年均能生成对应快照；快照载荷可用于审计和单组恢复 | 等用户确认开始 |
| I7-04 | 实现单组快照恢复服务 | P0 | 未开始 | `I7-02`、`I7-03` | `admin-rollback/restore-group-snapshot` 接口、服务层、并发写锁、回退日志 | 管理员可选择单组快照恢复到目标阶段；系统生成安全快照，目标节点之后结果失效但保留；订单归属不释放；并发提交返回状态变化提示 | 等用户确认开始 |
| I7-05 | 实现管理员端“回退与修正”页面 | P1 | 未开始 | `I7-04` | 管理员路由、导航、`退回重提` Tab、`恢复快照` Tab、确认弹窗 | 管理员可在统一页面执行退回重提、查询快照、创建手动快照、恢复单组快照，并看到风险提示与操作结果 | 等用户确认开始 |
| I7-06 | 接入年度控制阻断、玩家提示与汇总待重提展示 | P0 | 未开始 | `I7-04`、`I7-05` | 年度控制接口与页面、玩家经营/财报提示、汇总页状态展示 | 存在回退补提 / 待重提小组时管理员不能开放下一年；玩家端清楚提示被回退到的年份阶段；汇总页对失效结果显示 `待重提` 且不计入排名 | 等用户确认开始 |
| I7-07 | 回归测试与人工验收清单 | P0 | 未开始 | `I7-02` ~ `I7-06` | 服务层测试、接口集成测试、前端构建、人工验收清单 | 覆盖本年退回重提、跨年单组快照恢复、订单不释放、奖惩失效、汇总待重提、破产恢复、开放下一年阻断、并发提交冲突等关键场景 | 等用户确认开始 |
