...
| I4-01 | 明确线上订单与市场竞标正式需求口径 | P0 | 已完成 | 用户 2026-05-18 确认、`道具-订单推算（服务企业）.xlsx` | `docs/requirements_spec.md`、`docs/requirements_consensus_checklist.md`、`docs/minimal_state_machine.md`、`docs/calculation_rule_spec.md`、`docs/excel_field_mapping.md`、`docs/api_design.md`、`docs/database_design.md` | 已明确 `0年` 不需要订单、`1年` 起按本地/区域/全国/全球四个市场分别抢单；每市场四类订单；每市场只选一轮，每组最多一单；`1年` 按市场投入排序，`2年` 起市场龙头优先；剩余订单不强制选完；未交付只记录状态，处罚规则待确认 | 已完成文档沉淀，尚未进入代码实现 |
| I4-02 | 实现管理员订单管理：Excel 上传、数量配置、订单池生成 | P0 | 未开始 | `I4-01`、`docs/api_design.md`、`docs/database_design.md` | 后端 `admin-order` 接口、订单 Excel 解析器、订单池迁移脚本、管理员订单管理页 | 管理员可上传订单推算 Excel，按 `年份 + 市场 + 订单类型` 配置数量，生成固定订单池，并限制已开放/已选单市场不能覆盖 | 待开发 |
| I4-03 | 实现玩家年度订单页：市场投入、选单顺序、选择订单 | P0 | 未开始 | `I4-02`、`docs/minimal_state_machine.md` | 后端 `player-order` 接口、玩家年度订单页、市场投入与选单状态展示 | 玩家可提交各市场投入，按系统顺序选择订单；未投入市场不参与；每市场每组最多选一单；选择后不可撤销 | 待开发 |
| I4-04 | 打通订单模块与经营页规则联动 | P0 | 未开始 | `I4-03`、`docs/calculation_rule_spec.md` | 经营页查询/提交规则改造、订单总额与市场投入汇总、订单交付状态回写 | 正式年份未完成订单前置时拦截 `Q1` 提交；已选订单金额汇总进入订单总额；市场投入汇总进入市场投入；订单交付/未交付状态可记录 | 待开发 |
| I4-05 | 补订单模块测试、联调脚本与原型/页面验收 | P1 | 未开始 | `I4-02`、`I4-03`、`I4-04` | 集成测试、`.http` 脚本、管理员/玩家页面验收清单 | 覆盖 `0年` 无订单、无投入市场跳过、投入并列随机、市场龙头优先、每市场一轮选单、Q1 前置拦截、未交付状态保留等场景 | 待开发 |
