...
- 未开始：`0`
- 阻塞：`0`
- 当前状态：`I13-02 ~ I13-07、I13-09 ~ I13-10、I14-01、I15-01 ~ I15-04、I16-01 ~ I16-03 已完成；订单竞标市场自动聚焦与结束态展示优化已完成；I13-08 订单专项造数命令已进入手动版实现，待继续完善`
...
| I16-03 | 订单交付自动写入销售收入与回退 / 快照草稿化改造 | P0 | 已完成 | 用户 2026-07-29 确认订单交付新口径、退回重提与恢复快照均恢复为可修改草稿 | 后端交付接口、订单交付历史 / 修订记录、经营页查询 / 保存 / 提交联动、玩家订单页状态展示、回退与快照恢复链路、相关测试与验收清单 | 已实现正式年份订单交付不再校验玩家手填销售收入；交付成功后按当前有效交付订单金额汇总写入经营页销售收入；经营页销售收入只读并由后端在查询、保存、提交、财报计算和快照生成前强制覆盖；已交付订单不消失而是灰色展示；退回重提和恢复快照后旧交付写修订留痕并失效，订单恢复待交付，可重新交原订单或换交其他已选订单；销售收入只按重新确认后的当前有效交付生成 | 无；待用户在真实页面复测订单交付、回退和快照恢复 |

### 9.19 I17：经营页项目进度、新市场培育、资质认证升级

> 最新口径以 `docs/requirements_spec.md` 6.2.6 ~ 6.2.8、`docs/calculation_rule_spec.md` 3.6.2 ~ 3.6.4 和 `docs/minimal_state_machine.md` 8.5 为主；本迭代只实现经营页记录、三市场培育费用 / 累计 / 锁定、资质认证跨年继承，不接入订单市场开启、订单池、龙头、选单顺序或管理员审核流程。

| Task ID | 任务 | 优先级 | 状态 | 依赖 | 建议交付物 | 完成标准 | 阻塞情况 |
|---|---|---|---|---|---|---|---|
| I17-01 | 写入开发任务清单并冻结本轮实现边界 | P0 | 已完成 | 用户 2026-07-30 确认“写开发任务清单，然后直接开始开发” | `docs/implementation_plan.md` | 已拆分 I17-01 ~ I17-06；明确本轮不新增订单联动、不新增管理员审核、不修改 Excel 源文件；优先采用经营 payload 结构化扩展并兼容旧草稿 | 无 |
| I17-02 | 扩展经营 payload 数据结构、默认值、归一化与旧草稿兼容 | P0 | 已完成 | I17-01 | `internal/model/payload/operating_payload.go`、`frontend/src/types/sandbox-game.ts` | 已新增 `projectProgressUpdate`、`marketCultivation`、`qualificationCertification` 三块结构；旧 `assetAdjustment.marketCultivation` 继续作为兼容兜底；空 payload 和 clone/normalize 不产生 `null`；显式新结构 `0` 不回退旧字段 | 无 |
| I17-03 | 实现后端新市场培育费用、累计、解锁和资质继承推导 | P0 | 已完成 | I17-02 | 后端经营查询 / 保存 / 提交链路、经营与财报计算器、必要仓储辅助方法 | 已实现新市场培育只按本年三市场实际投入合计计费；累计按上一年有效经营链递推；解锁采用 `>=`；已解锁市场后续年份锁定且有效投入为 `0`；资质解锁状态按上一年有效状态继承锁定 | 无 |
| I17-04 | 补充后端输入校验与回退 / 快照待重提兼容 | P0 | 已完成 | I17-03 | `internal/service/manual_integer_validation.go`、`internal/service/operating_feature_state.go`、相关测试 | 已校验市场培育年度投入只允许 `0 / 1`；已锁定市场拒绝继续投入；资质状态只允许 `未解锁 / 解锁`；项目进度不参与公式，回退 / 快照旧值仍按现有失效草稿机制保留展示 | 无 |
| I17-05 | 实现玩家端经营页三块 UI 和前端预览同步 | P0 | 已完成 | I17-02 ~ I17-04 | `OperatingSheet.vue`、`sandbox-game-operating-preview.ts`、`player-operating.ts` | 已实现项目进度记录、新市场培育三市场投入 / 累计 / 状态 / 锁定展示、资质认证四项状态下拉；已按 Excel 版式返工为表格单元格，其中项目进度按生产厂房 A/B/C × 第一至第四季度 × 产线/进度展示，并按季度 scope 控制可编辑；新市场培育和资质认证保留年末可填、Q1 等季度锁定口径；现金支出、综合费用、财报预览等均只按本年有效投入合计计算，不因已解锁状态重复扣钱 | 无 |
| I17-06 | 自动化验证、前后端构建与计划回写 | P0 | 已完成 | I17-05 | 后端单元 / 服务测试、前端 build、`docs/implementation_plan.md` | 已覆盖旧字段兼容、三市场费用不重复扣钱、显式 `0` 不回退旧字段、锁定拒填、资质状态收紧；验证通过 `go test ./... -count=1`、`cd frontend && npm run build`、`git diff --check` | 无 |
| I17-07 | 修正项目进度更新为 Excel `4/3/1` 厂房槽位结构 | P0 | 已完成 | 用户 2026-08-03 截图确认、`I17-05` | `docs/requirements_spec.md`、`docs/calculation_rule_spec.md`、`game doc/Excel计算规则与跨表联动说明.md`、`frontend/src/types/sandbox-game.ts`、`frontend/src/components/sandbox-game/player/OperatingSheet.vue`、`internal/model/payload/operating_payload.go`、相关校验测试 | 已将项目进度从当前 `生产厂房 A/B/C × 四季度` 的 `12` 条记录修正为 Excel 的 `生产厂房 A=4 槽 / B=3 槽 / C=1 槽 × 四季度` 共 `32` 条记录；旧 `12` 条记录兼容映射到对应厂房第 `1` 槽；产线限定为空 / 人工 / 半自动 / 自动 / 智能，进度限定为空 / `0~4`；仍只做当年留痕，不参与公式、不跨年、不自动移动项目交付库 | 无 |

### 12.1 本轮新增记录

| 日期 | 记录 |
|---|---|
| 2026-08-03 | 任务状态：✅ 已完成（I17-07 项目进度 4/3/1 槽位修正）；落点：`docs/requirements_spec.md`、`docs/requirements_consensus_checklist.md`、`docs/calculation_rule_spec.md`、`game doc/Excel计算规则与跨表联动说明.md`、`frontend/src/types/sandbox-game.ts`、`frontend/src/components/sandbox-game/player/OperatingSheet.vue`、`internal/model/payload/operating_payload.go`、`internal/model/payload/operating_payload_test.go`、`internal/service/operating_feature_state.go`、`internal/service/manual_integer_validation_test.go`、`docs/implementation_plan.md`；偏差说明：按用户截图确认，原 Excel 中 `3.项目进度更新` 不是每个厂房一行，而是 `生产厂房 A=4 个槽位 / B=3 个槽位 / C=1 个槽位`。本轮将页面与 payload 修正为 `厂房 × 槽位 × 季度` 共 `32` 条记录，旧版 `A/B/C × 四季度` 共 `12` 条记录兼容映射到对应厂房第 `1` 槽；产线与进度补充取值校验；仍保持只做当年留痕、不参与公式、不跨年、不自动移动项目交付库。验证：`go test ./internal/model/payload ./internal/service -run 'TestOperatingProjectProgress|TestValidateOperatingManualIntegers' -count=1`、`go test ./internal/model/payload ./internal/service -count=1`、`go test ./... -count=1`、`cd frontend && npm run build`、`git diff --check` 均通过，`git diff --check` 仅提示 Windows 换行转换警告。下一步：用户重启 / 刷新前端后，在经营页 Q1/Q2/Q3/Q4 分别验收 A 厂房 4 行、B 厂房 3 行、C 厂房 1 行，以及对应季度可编辑、其他季度锁定。 |
| 2026-07-30 | 任务状态：✅ 已完成（资质认证单元格顺序修正）；落点：`frontend/src/components/sandbox-game/player/OperatingSheet.vue`、`docs/implementation_plan.md`；偏差说明：按用户截图反馈修正 `8.资质认证` 行的 Excel 版式，由“4 个资质名称集中展示 + 4 个填写框集中展示”改为“每个资质名称后紧跟自己的填写框”，保持年末可填、季度锁定、继承解锁后锁定等既有业务规则不变。验证：`cd frontend && npm run build` 通过。下一步：用户刷新前端后核对资质认证行是否与 Excel 截图一致。 |
| 2026-07-30 | 任务状态：✅ 已完成（I17 前端 Excel 版式返工）；落点：`frontend/src/components/sandbox-game/player/OperatingSheet.vue`、`frontend/src/types/sandbox-game.ts`、`docs/implementation_plan.md`；偏差说明：按用户截图反馈把玩家经营页 `3.项目进度更新 / 7.新市场培育 / 8.资质认证` 从网页卡片式改回贴近 Excel 的表格单元格样式。项目进度由“全年只要有任意阶段可编辑就能填”修正为按季度 scope 控制，页面展示为生产厂房 A/B/C × 第一至第四季度 × 产线/进度；新市场培育和资质认证确认属于年末工作，因此 Q1 不能填保持不变，只调整展示为 Excel 表格；本次不修改订单联动、管理员审核、后端公式边界。验证：`cd frontend && npm run build` 通过。下一步：用户刷新前端，在 Q1 与 YEAR_END 阶段分别验收三块区域的版式和锁定状态。 |
| 2026-07-30 | 任务状态：✅ 已完成（I17 代码开发）；落点：`internal/model/payload/operating_payload.go`、`internal/model/payload/operating_payload_test.go`、`internal/rules/operating/operating_calculator.go`、`internal/rules/report/report_calculator.go`、`internal/service/operating_feature_state.go`、`internal/service/manual_integer_validation.go`、`internal/service/manual_integer_validation_test.go`、玩家经营 / 财报查询与提交链路、`frontend/src/types/sandbox-game.ts`、`frontend/src/components/sandbox-game/player/OperatingSheet.vue`、`frontend/src/stores/player-operating.ts`、`frontend/src/utils/sandbox-game-operating-preview.ts`、`frontend/src/views/sandbox-game/player/operating/PlayerOperatingPage.vue`、`docs/implementation_plan.md`；偏差说明：按用户确认完成 `3.项目进度更新 / 7.新市场培育 / 8.资质认证` 开发，不接入订单市场开启、龙头、选单顺序或管理员审核。新市场培育新增三市场结构并兼容旧 `assetAdjustment.marketCultivation`：只有新结构有显式输入 / 累计 / 锁定状态时才作为权威金额，避免旧草稿被空新结构吞成 `0`；显式填写 `0` 则不回退旧字段。已解锁市场后续年份锁定且本年有效投入为 `0`，不会因已解锁状态重复扣钱；资质认证只允许 `未解锁 / 解锁`，上一年有效解锁后继承并锁定。验证：`go test ./... -count=1`、`cd frontend && npm run build`、`git diff --check` 均通过，`git diff --check` 仅输出 Windows 换行提示。下一步：用户重启前后端后，在经营页人工验收三市场投入、提交后锁定、下一年继承不重复扣钱、资质继承锁定和回退 / 快照失效草稿展示。 |
| 2026-07-29 | 任务状态：✅ 已完成（订单交付列表账期展示）；落点：`frontend/src/views/sandbox-game/player/order/PlayerOrderPage.vue`、`docs/implementation_plan.md`；偏差说明：按用户反馈补充玩家端“本组订单交付”区域的信息密度，不修改订单交付、销售收入生成、回退 / 快照恢复、订单归属或竞标规则。待交付勾选列表和已交付订单列表均在同一行展示“金额 X · 账期 X 季度”，方便玩家在提交订单前直接核对账期。验证：`cd frontend && npm run build` 通过。下一步：用户刷新前端，在订单交付面板核对待交付订单和已交付订单是否都能看到对应账期。 |
| 2026-07-29 | 任务状态：✅ 已完成（I16-03 复测修正）；落点：`internal/repository/order_repository.go`、`internal/service/player_operating_command_service.go`、`internal/service/player_operating_command_service_test.go`、`frontend/src/views/sandbox-game/player/order/PlayerOrderPage.vue`、`game doc/Excel计算规则与跨表联动说明.md`、`docs/order_module_acceptance_checklist.md`、`docs/rollback_module_acceptance_checklist.md`、`docs/implementation_plan.md`；偏差说明：按用户确认的方案 A 修复多次回退 / 恢复快照后旧交付失效状态累计展示问题。“交付已失效”定义为上次经营提交结果被回退 / 快照恢复撤销后的临时操作状态；玩家重新提交季度后，未再次交付的旧失效订单恢复普通待交付；再次回退 / 恢复快照只根据上次重新提交后仍然有效的交付订单生成新的失效状态。历史留痕仍保留在订单交付修订记录和快照 / 回退日志中，玩家订单页不累计展示所有历史失效。验证：`go test ./internal/service -run TestSubmitOperatingStageClearsStaleInvalidatedOrderDeliveries -count=1` 通过。下一步：用户重启后端并刷新前端，复测“第一次回退 4 个失效 → 重新交付 2 个并提交 → 第二次回退只显示 2 个失效”。 |
| 2026-07-29 | 任务状态：✅ 已完成（I16-03 代码开发）；落点：`migrations/mysql/0017_order_delivery_revenue.sql`、`cmd/dbtool/main.go`、`internal/enum/order.go`、`internal/model/entity/order.go`、`internal/repository/order_repository.go`、`internal/service/order_operating_link_service.go`、`internal/service/player_operating_command_service.go`、`internal/service/player_order_service.go`、`internal/service/admin_control_command_service.go`、`internal/service/admin_rollback_service.go`、`internal/service/rollback_snapshot_service.go`、`internal/http/handler/player_order_handler.go`、`internal/service/order_workflow_integration_test.go`、`internal/service/player_operating_command_service_test.go`、`frontend/src/types/sandbox-game-order.ts`、`frontend/src/stores/player-order.ts`、`frontend/src/components/sandbox-game/player/OperatingSheet.vue`、`frontend/src/views/sandbox-game/player/order/PlayerOrderPage.vue`、`docs/implementation_plan.md`；偏差说明：按用户确认的新口径完成订单交付自动写入销售收入开发，不改变订单归属、订单池、选单顺序、市场龙头或已结束轮次。后端新增订单交付修订表，交付成功写 `DELIVERED` 修订记录并返回当前季度销售收入；经营页查询 / 保存 / 提交、财报计算和快照生成前均按当前有效交付记录覆盖正式年份销售收入；退回重提和恢复快照都会将目标阶段及之后的有效交付置为失效草稿并写 `INVALIDATED` 修订记录，订单恢复待交付但保留旧交付季度提示；前端经营页销售收入在订单联动开启后变为只读系统项，订单页保留待交付、已交付、交付已失效三类状态展示。验证：`go test ./internal/service -run TestOrderWorkflowCoversGenerationSequenceSelectionDeliveryAndUnfinished -count=1`、`go test ./internal/service -count=1`、`go test ./... -run TestNonExisting -count=0`、`cd frontend && npm run build` 均通过。下一步：用户重启后端并刷新前端，在真实页面复测“交付订单 → 经营页销售收入自动更新 → 退回 / 快照恢复后交付失效并可重新交付”。 |
| 2026-07-29 | 任务状态：✅ 已完成（文档冻结）；落点：`docs/requirements_spec.md`、`docs/requirements_consensus_checklist.md`、`docs/calculation_rule_spec.md`、`docs/api_design.md`、`docs/database_design.md`、`docs/excel_field_mapping.md`、`docs/minimal_state_machine.md`、`docs/order_multi_round_upgrade_design.md`、`docs/order_module_acceptance_checklist.md`、`docs/rollback_module_acceptance_checklist.md`、`game doc/Excel计算规则与跨表联动说明.md`、`docs/implementation_plan.md`；偏差说明：本轮只按用户确认完成订单交付自动写入销售收入与回退 / 快照草稿化口径文档，不修改代码。已明确正式年份订单交付不再校验玩家手填销售收入，交付成功后由系统按当前有效交付订单金额汇总生成经营页销售收入；经营页销售收入为只读系统联动项，后端在查询、保存、提交和快照生成前均应覆盖；已交付订单在订单页灰色展示不消失；退回重提和恢复快照均恢复为待重提 / 失效草稿，旧交付和旧销售收入只作历史留痕，订单恢复待交付，玩家可重新交原订单或改交其他已选订单。下一步：进入 `I16-03` 代码开发，重点补交付历史 / 修订记录、经营页销售收入重建服务、订单页状态展示、回退与快照恢复链路和回归测试。 |
| 2026-07-30 | 任务状态：✅ 已完成（文档收口）；落点：`docs/requirements_spec.md`、`docs/requirements_consensus_checklist.md`、`docs/calculation_rule_spec.md`、`docs/minimal_state_machine.md`、`docs/excel_field_mapping.md`、`game doc/Excel计算规则与跨表联动说明.md`、`docs/implementation_plan.md`；偏差说明：本轮仅根据用户确认把 `3.项目进度更新 / 7.新市场培育 / 8.资质认证` 三项规则写回正式文档，不进入代码实现。已明确：项目进度只做当年留痕，不参与计算、不跨年；新市场培育按区域/全国/全球三项分别记录，按本年实际投入计费，累计只用于解锁并且采用 `>=` 业务语义，不因已解锁状态重复扣钱；资质认证只记录 `未解锁/解锁`，已解锁状态跨年继承并锁定，不参与任何计算；同时补充新 Excel 模板中 `1年经营!N63` 的全球累计引用风险，明确系统实现按业务语义修正。下一步：如用户确认，可继续拆分为开发任务并进入后端 payload / 计算 / 页面实现。 |
| 2026-07-28 | 任务状态：✅ 已完成；落点：`docs/admin_order_pool_group_filter_design.md`、`docs/README.md`、`internal/http/dto/admin_order_dto.go`、`internal/http/handler/admin_order_handler.go`、`internal/repository/order_repository.go`、`internal/service/admin_order_service.go`、`frontend/src/api/sandbox-game/admin-order.ts`、`frontend/src/types/sandbox-game-admin.ts`、`frontend/src/stores/admin-order.ts`、`frontend/src/views/sandbox-game/admin/order/AdminOrderPage.vue`、`docs/implementation_plan.md`；偏差说明：按用户确认在管理员端订单池查看区新增小组筛选能力，不新增独立小组订单页，不修改订单生成、确认、选单、交付、市场龙头、订单锁定或历史订单事实。筛选支持“全部订单 / 全部已选小组 / 指定小组”，并可与年份、市场、订单类型叠加使用；后端订单池查询补充 `selectedOnly / selectedGroupId` 参数和 `selectedGroupName / groupOptions` 返回，前端表格“选中组”优先展示小组名称。验证：`cd frontend && npm run build`、`go test ./internal/service -count=1` 通过。下一步：用户在管理员订单池页按“全部已选小组”和具体小组筛选，确认仅展示已选订单且不影响原全部订单查询。 |
| 2026-07-28 | 任务状态：✅ 已完成；落点：`docs/order_market_auto_focus_design.md`、`internal/service/player_order_service.go`、`frontend/src/types/sandbox-game-order.ts`、`frontend/src/views/sandbox-game/admin/order/AdminOrderPage.vue`、`frontend/src/views/sandbox-game/player/order/PlayerOrderPage.vue`、`docs/implementation_plan.md`；偏差说明：按用户确认补充“选单结束态展示”体验，不修改订单池、选单顺序、订单归属、交付、市场龙头或数据库事实状态。管理员端在选中 `COMPLETED` 标段时新增绿色结束提示，说明结束原因、已选订单和剩余订单；完成态隐藏“代跳过原因 / 跳过当前小组”；标段状态支持显示 `已完成（订单池已空）` 等原因；当标段已完成且完成原因为订单池选空、后续顺序仍为 `WAITING / CURRENT` 时，管理员端和玩家端均展示为 `未执行（订单池已空）`，避免误解为仍待选择。玩家端补充 `completionReason` 返回字段，并在完成态展示本组是否获得订单。验证：`cd frontend && npm run build`、`go test ./internal/service -run TestOrderWorkflowCoversGenerationSequenceSelectionDeliveryAndUnfinished -count=1`、`git diff --check` 通过；`git diff --check` 仅提示 Windows 换行转换警告。下一步：用户在订单池被选空的已完成标段复测管理员顺序表，确认后续小组/后续轮次显示“未执行（订单池已空）”，并确认完成态不再显示跳过当前小组入口。 |
| 2026-07-28 | 任务状态：✅ 已完成；落点：`docs/order_market_auto_focus_design.md`、`docs/README.md`、`frontend/src/stores/player-order.ts`、`frontend/src/stores/admin-order.ts`、`frontend/src/views/sandbox-game/admin/order/AdminOrderPage.vue`、`docs/implementation_plan.md`；偏差说明：按用户确认先写文档再开发“订单竞标市场自动聚焦”优化，不修改订单释放顺序、选单顺序、订单池、订单归属、交付、市场龙头或历史订单事实。文档明确 `SELECTING > ROUND_READY > SEQUENCE_READY / WAITING_RELEASE` 的关注标段优先级，并以 `releaseSequenceNo` 作为同优先级排序依据；玩家端在初始化/刷新时强跟随真实竞标市场，静默轮询时仅在当前市场无流程关注点时切换；管理员端在加载、刷新和轮询时并发读取全部市场竞标状态，自动把“控制市场”切到真实当前或下一步应处理的市场，手动切换控制市场时本次加载不立即被自动聚焦抢回。验证：`cd frontend && npm run build`、`go test ./internal/service -run TestOrderWorkflowCoversGenerationSequenceSelectionDeliveryAndUnfinished -count=1` 通过。下一步：用户在区域市场 / 代办过检 `SELECTING` 时刷新管理员端和玩家端，确认页面保持区域市场；在 `ROUND_READY` 时确认不跳走；在当前市场全部完成后确认自动切到下一个释放顺序所在市场。 |
| 2026-07-28 | 任务状态：✅ 已完成；落点：`internal/service/player_order_service.go`、`internal/service/order_workflow_integration_test.go`、`frontend/src/stores/admin-order.ts`、`frontend/src/views/sandbox-game/admin/order/AdminOrderPage.vue`、`frontend/src/stores/player-order.ts`、`frontend/src/views/sandbox-game/player/order/PlayerOrderPage.vue`、`docs/implementation_plan.md`；偏差说明：用户人工验证“开启下一轮”后第三组可正常选单，说明多轮核心状态机可用，本轮只修复 `ROUND_READY` 等待下一轮阶段的展示和按钮误导，不修改选单顺序、订单池、订单归属或轮次推进规则。后端市场状态汇总补充 `ROUND_READY`，避免市场仍有下一轮待开启时显示“已完成”；管理员端在存在 `SELECTING / ROUND_READY` 当前标段时禁用“释放下一个标段”，并给出需先完成当前标段/开启下一轮的提示；玩家端当前展示标段补充 `ROUND_READY`，等待下一轮时继续展示真实当前标段，并将操作提示改为“当前轮已结束，等待管理员开启下一轮”。验证：`go test ./internal/service -run TestOrderWorkflowCoversGenerationSequenceSelectionDeliveryAndUnfinished -count=1`、`cd frontend && npm run build` 通过；首次直接执行 `frontend/npm.cmd run build` 因未跟踪的 `frontend/npm.cmd` 不是当前系统有效可执行程序失败，已改用系统 `npm` 验证通过。下一步：用户刷新管理员和第三组玩家端页面，复测两舱贵宾 `ROUND_READY` 阶段市场状态、释放按钮禁用、玩家端当前标段和等待提示。 |
| 2026-07-28 | 任务状态：✅ 已完成；落点：`internal/service/player_order_service.go`、`frontend/src/stores/admin-order.ts`、`internal/service/order_workflow_integration_test.go`、`docs/implementation_plan.md`；偏差说明：修复管理员端多轮竞标中第一轮完成后标段进入 `ROUND_READY`，但页面“开启下一轮”按钮不可点击的问题。根因是查询接口只在 `SELECTING` 时返回 `currentSegment`，而前端按钮又只依赖 `currentSegment`，导致 `ROUND_READY` 标段虽然在列表中有 `nextRoundNo`，顶部状态卡和按钮仍认为当前无可操作标段。本轮将 `ROUND_READY` 也纳入管理员当前可操作标段，前端 store 增加按 `SELECTING -> ROUND_READY` 的兜底选择；新增回归断言第一轮结束后查询接口必须返回 `ROUND_READY` 当前标段和第 2 轮 `nextRoundNo`。验证：`go test ./internal/service -run 'TestOrderWorkflowCoversGenerationSequenceSelectionDeliveryAndUnfinished|TestOrderPassKeepsLaterRoundEligibility' -count=1`、`go test ./... -count=1`、`frontend/npm.cmd run build`、`git diff --check` 均通过；`git diff --check` 仅提示 Windows 换行转换警告。下一步：用户刷新管理员订单页后，在本地市场代办过检第一轮结束状态下确认“当前 / 下一轮”显示下一轮并可点击“开启下一轮”。 |
...
| 2026-07-31 | 任务状态：✅ 已完成；落点：`docs/admin_operation_year_scope_design.md`、`docs/requirements_spec.md`、`docs/requirements_consensus_checklist.md`、`docs/api_design.md`、`docs/rollback_module_acceptance_checklist.md`、`docs/adjustment_module_acceptance_checklist.md`、`internal/service/admin_operation_context.go`、`internal/service/admin_group_data_query_service.go`、`internal/http/dto/admin_group_data_dto.go`、`internal/http/handler/admin_group_data_handler.go`、`internal/app/router.go`、`internal/http/dto/admin_control_dto.go`、`internal/http/handler/admin_control_handler.go`、`internal/service/admin_control_command_service.go`、`internal/service/admin_rollback_service.go`、`internal/http/dto/admin_notice_dto.go`、`internal/http/handler/admin_notice_handler.go`、`internal/service/admin_notice_command_service.go`、`frontend/src/types/sandbox-game-admin.ts`、`frontend/src/api/sandbox-game/admin-group-data.ts`、`frontend/src/stores/admin-rollback.ts`、`frontend/src/views/sandbox-game/admin/AdminRollbackPage.vue`、`frontend/src/stores/admin-notice.ts`、`frontend/src/views/sandbox-game/admin/notice/AdminNoticePage.vue`、`internal/service/admin_notice_integration_test.go`、`docs/implementation_plan.md`；偏差说明：按用户确认完成管理员操作年份收口。新增管理员小组操作上下文接口，统一按“最早待重提年份优先，否则当前开放年”解析目标小组当前操作年份；退回重提页面删除可编辑年份，后端允许旧接口兼容传 `yearNo`，但传错会拒绝，待重提链路允许在已开放下一年后继续补提目标年份；恢复快照页面删除恢复原因和确认文本，服务端默认记录“管理员恢复快照”，保留安全快照、回退日志、待重提、汇总撤回、奖惩恢复、订单交付失效等原安全逻辑；奖惩下发页面删除年份下拉，只读展示系统归属年份和阶段，后端预览/下发均按目标小组当前操作年份与当前可编辑阶段解析并拒绝错传历史年份。测试侧同步修正奖惩集成测试夹具，使其显式处于 0 年操作上下文，避免被本地数据库当前开放年污染。验证：`go test ./internal/service/...`、`go test ./...`、`cd frontend && npm run build` 均通过；`frontend\npm.cmd run build` 未执行成功，原因是仓库中未跟踪的 `frontend/npm.cmd` 为 0 字节空文件，已改用系统 npm 验证。下一步：用户重启后端并刷新前端，人工验收退回重提页年份只读、快照恢复免原因/确认文本、奖惩下发页无法选择历史年份。 |
| 2026-08-03 | 任务状态：✅ 已完成；落点：`scripts/build-competition-package.ps1`、`docs/implementation_plan.md`；偏差说明：部署场景 B 下用户确认正式打包脚本需要补齐订单交付自动写销售收入相关迁移。本轮仅修正正式比赛包构建清单，将 `migrations/mysql/0017_order_delivery_revenue.sql` 纳入打包产物；`cmd/dbtool/main.go` 原本已经会执行 `0017`，本次不改变数据库迁移顺序、不清理现有比赛数据、不调整单/双部署入口。下一步：先确认数据库备份脚本和单实例部署口径，再决定是否新增管理员双击备份入口与收口部署脚本。 |
| 2026-08-03 | 任务状态：✅ 已完成；落点：`docs/requirements_spec.md`、`docs/api_design.md`、`docs/order_module_acceptance_checklist.md`、`docs/calculation_rule_spec.md`、`docs/implementation_plan.md`、`frontend/src/views/sandbox-game/player/order/PlayerOrderPage.vue`；偏差说明：按用户确认完成订单未交付展示规则与页面实现：`UNFINISHED` 订单只在所属年份订单页展示，不滚入后续年份、不允许后续年份补交；玩家端不新增独立未交付订单区域，而是在所属标段“本组已选订单”原行红色标记“未交付”，并提示“年末仍未交付，不能在后续年份补交”。验证：`cd frontend && npm run build` 已通过。下一步：用户在已产生 `UNFINISHED` 的年份订单页人工验收红色留痕展示。 |
| 2026-08-03 | 任务状态：✅ 已完成；落点：`docs/competition_backup_single_deploy_plan.md`、`docs/README.md`、`docs/implementation_plan.md`；偏差说明：按用户最新确认完成“场景 B 数据备份与单实例新目录部署”方案文档。本方案明确管理员电脑已有旧比赛数据时不覆盖旧代码目录，先在当前部署目录生成/复用 `backup` 文件夹并导出包含完整 SQL、备份说明、结果摘要 CSV 的 zip；新代码部署到新文件夹并收口为单实例，一个启动入口、一个数据库、一个访问地址，生产制造版 / 贵宾服务版由网页赛前配置页选择版本包；同时记录当前最新两个提交 `0854aae 修正项目进度厂房槽位结构` 与 `06d4c7a 优化订单未交付展示` 必须纳入部署基线，后续开发任务包含删除未跟踪空文件 `frontend/npm.cmd`、新增可复用备份脚本、双击备份入口、打包纳入备份工具并更新现场部署清单。下一步：用户确认后进入脚本开发与部署手册更新。 |
| 2026-08-03 | 任务状态：✅ 已完成；落点：`scripts/backup-competition-database.ps1`、`scripts/build-competition-package.ps1`、`scripts/competition-launchers/首次初始化数据库.bat`、`scripts/competition-launchers/启动沙盘系统.bat`、`scripts/competition-launchers/重启沙盘系统.bat`、`scripts/competition-launchers/停止沙盘系统.bat`、`scripts/competition-launchers/备份当前比赛数据库.bat`、`scripts/competition-launchers/赛前重置当前比赛.bat`、`scripts/competition-launchers/README.md`、`scripts/competition-launchers/安装部署手册-正式版.txt`、`scripts/competition-launchers/操作手册-正式版.txt`、`docs/competition_backup_single_deploy_plan.md`、`docs/competition_deploy_checklist.md`、`docs/competition_launch_runbook.md`、`docs/README.md`、`docs/implementation_plan.md`；偏差说明：按用户确认完成 D1 ~ D12。保留并验证 `0017_order_delivery_revenue.sql` 正式打包；删除未跟踪空文件 `frontend/npm.cmd`；新增可复用数据库备份脚本，支持管理员输入文件名、当前部署目录自动创建/复用 `backup` 文件夹、完整 SQL 备份、结果摘要 CSV、备份说明 TXT、zip 打包、同名自动追加时间戳、SQL 成功但摘要失败时仍保留备份；新增单实例管理员入口，正式包根目录提供首次初始化、启动、重启、停止、备份、赛前重置脚本，不再让管理员通过脚本选择制造业版 / 服务版，版本由网页赛前配置页选择；现场部署清单和管理员手册已同步为“场景 B 先备份旧库，新代码部署到新文件夹，新比赛使用新数据库”。验证：PowerShell 语法解析通过，`git diff --check` 通过；已执行 `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build-competition-package.ps1 -ConfigPath .\configs\competition.yaml -OutputDir .\.runtime\competition-package-check`，前端 `npm run build`、Go 可执行文件构建与正式包压缩均成功；已核对产物包含 `0017`、备份脚本、单实例 bat、两份管理员手册和部署方案文档。未执行真实 `mysqldump` 数据库导出，避免触碰当前比赛库，后续需在管理员电脑有 MySQL 客户端环境时人工验收备份 zip 和摘要 CSV。下一步：如用户确认，可提交并推送本次部署工具改动。 |
