...
| I3-05 | 补管理员赛前初始化回归验证与运行说明 | P1 | 部分完成 | `I3-04` | 测试文档、联调手册、验收清单 | 已补后端集成测试与管理员默认路由回归，当前已通过 `go test ./...` 与 `frontend/npm run build`；仍需补齐 `1 / 6 / 10` 组三种初始化验收与独立运行说明 | 继续补测试样例与运行文档 |
