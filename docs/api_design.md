...
- 服务端不再校验玩家手填销售收入是否等于交付订单金额；交付成功后订单状态变为当前有效 `DELIVERED`。
- 服务端按该季度当前有效已交付订单金额合计自动覆盖经营页该季度销售收入；同一季度多次交付时，销售收入为该季度全部当前有效交付金额累计值，不是本次金额覆盖。
- 返回结果建议包含 `deliveredAmount`（本次交付金额合计）和 `stageSalesRevenue`（该季度当前有效销售收入合计）。
- 年末仍未交付订单标记为 `UNFINISHED`，首版不自动处罚。
- `UNFINISHED` 订单仍归属原选单年份，不进入后续年份订单页，不允许在后续年份通过交付接口补交；如需修改，只能通过退回 / 恢复快照回到订单所属年份的可编辑经营季度。
- 玩家订单页不新增单独未交付列表，接口返回的本组已选订单应在所属标段原位置展示 `UNFINISHED` 状态，前端文案统一为“未交付”。
...
```json
{
  ...
  "orderIds": [10001, 10002],
  "deliveredAmount": 57,
  "stageSalesRevenue": 57,
  "deliveredAt": "2026-07-29T10:00:00+08:00"
}
```
...
- 恢复单组快照不是把快照中的旧结果直接恢复为当前有效结果，而是以快照载荷作为草稿来源，让目标组进入待重提 / 失效草稿状态；玩家必须重新提交后才生成新的有效结果和新的自动快照。
- 普通通知不回退。
- 已确认破产不可通过退回重提或恢复快照撤销，已破产小组不得执行回退恢复为 `NORMAL`。
- 单组快照回退不释放已选订单、不重排历史选单顺序、不重算市场龙头、不重新生成已确认订单池。
- 目标阶段及后续阶段的旧订单交付记录保留为历史留痕 / 草稿参考，但不直接恢复为当前有效交付；对应订单恢复为待交付 / 可重新交付，玩家重新推进到对应季度后可以交付原订单，也可以改交其他仍归属本组的已选订单。
- 经营页销售收入只按重新确认后的当前有效交付记录生成；快照载荷和旧草稿中的历史销售收入不得直接进入新的正式提交。
...
生成单组经营阶段快照前，服务端必须先按当前有效订单交付记录覆盖经营页销售收入，保证快照中的经营页收入与订单交付状态自洽；手动单组快照如包含经营草稿，也应遵守同一覆盖口径。
...
| `ErrOrderDeliveryRevenueMismatch` | `422` | 历史废弃口径：季度销售收入与交付订单金额合计不一致 | 新口径不再使用该错误；交付成功后由系统自动生成季度销售收入 |
