...
> 说明：本文件已按正式版口径输出；字段显示名称仍可随最终 Excel 命名映射调整，但接口域划分、动作语义和结构性字段不应随意改动。
...
- 若首版采用项目内最小登录模块，也应保证业务接口层只依赖“已解析好的身份信息”，而不把登录逻辑耦合到业务域。
...
### 5.2 仍待最终 Excel 命名映射的字段

以下字段级内容后续可细修，但不影响接口整体结构：

- `operatingPayload` 内部键名的最终显示映射
- `reportManualPayload` 内部键名的最终显示映射
- 页面标签与字段名之间的映射表
